...
def perspective_to_orthographic(points_3d, alpha=1.0):
    """透視投影下の3次元座標を、正射投影カメラに対応する形へ座標変換する関数.
    ...
    Returns
    -------
        points_3d_ortho: 入力と同じ形状の正射投影変換後の3次元点群.
    ...
    X_ortho = alpha * (X / Z_safe)
    Y_ortho = alpha * (Y / Z_safe)
    # Zはそのまま保持
    points_3d_ortho = np.stack((X_ortho, Y_ortho, Z), axis=-1)
    return points_3d_ortho
...
def visualize_orthographic(rgb_image, depth_map, f, cx, cy, alpha=1.0, timestamp=None):
    """正射投影モードでのRGB画像と深度マップから生成した3D点群の可視化.
    ...
    # 2. 正射投影座標系への変換
    points_3d_ortho = perspective_to_orthographic(points_3d_persp, alpha=alpha)

    # 3. Filter points that are too far away
    flatten_points = points_3d_ortho.reshape(-1, 3)
    ...
    rr.log(
        "world/points3d_ortho",
        rr.Points3D(
            flatten_points * np.array([[1000.0, 1000.0, 1.0]]),
            colors=flatten_rgb,
        ),
    )
    # 参照用にRGB画像も記録
...
def main():
    ...
    parser.add_argument("--alpha", type=float, default=1.0, help="正射投影変換のスケール係数")
    parser.add_argument("--orthographic", action="store_true", help="正射投影モードを有効にする")
    parser.add_argument(
    ...
    if args.image_or_video.endswith(".mp4"):
        ...
        for i in tqdm(range(n_frames), desc="Visualizing video"):
            ...
            if args.orthographic:
                visualize_orthographic(
                    rgb_image, depth_map, args.f, args.cx, args.cy, args.alpha, timestamp=i / fps
                )
            else:
        ...
        if args.orthographic:
            visualize_orthographic(
                rgb_image, depth_maps, args.f, args.cx, args.cy, args.alpha, timestamp=0
            )
        else:
